<template>
    <div class="sidebar">
        ...
        <div class="boards">
            <span class="allboards">ALL BOARDS ({{ boards?.length }})</span>
            <div>
                <ul v-for="board in boards">
                    <SidebarItem :name="board.title" @click="changeBoardView(board.id)" />
                </ul>
...
<script setup lang="ts">
    ...
    const { data: boards, refresh: refreshBoards } = await useAsyncData(
        "boards",
        async () => {
    ...
    async function changeBoardView(url: string) {
        navigateTo(`/board/${url.toString()}`);
    }
    watch(
        boards || params || store,
        async () => {
