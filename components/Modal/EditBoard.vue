<template>
    <Modal v-if="store.modal.editBoard" :items="[]">
        <template #header>
        ...
        <template #form-content-input>
            ...
            <div v-for="(item, index) in store.activeBoard.category" :key="index">
...
<script lang="ts" setup>
    ...
    async function updateboard() {
        ...
        try {
            // await db.postCategory(boardId.value, store.inputItems);
        } catch (error) {
            console.error("Error updating board:", error);
            throw new Error();
        } finally {
